...
vec3 Iamb = vec3(0.5, 0.5, 0.5); // ambient light intensity
vec3 kd = vec3(0.9, 0.9, 0.05);     // diffuse reflectance coefficient
vec3 ka = vec3(0.45, 0.45, 0.025);   // ambient reflectance coefficient
vec3 ks = vec3(0.9, 0.9, 0.5);   // specular reflectance coefficient
vec3 lightPos = vec3(-1, 20, 5);   // light position in world coordinates
...
void main(void)
{
	...
	vec3 specularColor = I * ks * pow(max(0, NdotH), 250);
